...
void k2c_read_binary_float32_array_file_inplace(const char* filename, const size_t array_size, float* ptr)
{
...
void k2c_read_binary_float32_array_file_offset_limit(const char* filename, const size_t array_size, const size_t offset, float *pArray){
    size_t elements_read = 0;
    FILE *finp;
    //int foo;
    finp = fopen(filename, "rb");
    ...
    k2c_load_binary(pArray, finp, &elements_read, array_size, array_size);

    fclose(finp);
}

void k2c_load_binary(float *ptr, FILE *fptr, size_t *elements_read, const size_t n_elem_to_read, const size_t array_size){
