...
esp_err_t esp_psram_init(void)
{
    ...
    *----------------------------------------------------------------------------*/
    ...
    //previous code end
    //new code begin: 
        const uint32_t himem_reserved_size = esp_himem_reserved_area_size();
        const uint32_t previous_size = s_psram_ctx.regions_to_heap[PSRAM_MEM_8BIT_ALIGNED].size;
        if (s_psram_ctx.regions_to_heap[PSRAM_MEM_8BIT_ALIGNED].size > himem_reserved_size)
        {
            s_psram_ctx.regions_to_heap[PSRAM_MEM_8BIT_ALIGNED].size -= himem_reserved_size;
            s_psram_ctx.regions_to_heap[PSRAM_MEM_8BIT_ALIGNED].vaddr_end -= himem_reserved_size;
        }
        ESP_EARLY_LOGI(TAG, "[Castel] DISCOUNTING HIMEM %u : s_psram_ctx.regions_to_heap[PSRAM_MEM_8BIT_ALIGNED].size: %u, previous size: %u",
                            himem_reserved_size, s_psram_ctx.regions_to_heap[PSRAM_MEM_8BIT_ALIGNED].size, previous_size);
    //new code end
#endif
