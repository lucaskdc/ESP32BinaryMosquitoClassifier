...
int8_t fft_init_no_malloc(int size, fft_type_t type, fft_direction_t direction, fft_config_t *config, FPRECISION *input, FPRECISION *output, FPRECISION *twiddle_factors)
{
  int k,m;
  ...
  config->twiddle_factors = twiddle_factors;
  if (config->twiddle_factors == NULL)
    return 2;

  FPRECISION two_pi_by_n = TWO_PI / config->size;
  for (k = 0, m = 0 ; k < config->size ; k++, m+=2)
  {
    config->twiddle_factors[m] = cos(two_pi_by_n * k);    // real
    config->twiddle_factors[m+1] = sin(two_pi_by_n * k);  // imag
  }
...
// fft_config_t *fft_init(int size, fft_type_t type, fft_direction_t direction, FPRECISION *input, FPRECISION *output)
// {
//   /*
...
//   config->twiddle_factors = (FPRECISION *)malloc(2 * config->size * sizeof(FPRECISION));
...
//       config->input = (FPRECISION *)malloc(config->size * sizeof(FPRECISION));
//     else if (config->type == FFT_COMPLEX)
//       config->input = (FPRECISION *)malloc(2 * config->size * sizeof(FPRECISION));
...
//   {
//     if (config->type == FFT_REAL)
//       config->output = (FPRECISION *)malloc(config->size * sizeof(FPRECISION));
//     else if (config->type == FFT_COMPLEX)
//       config->output = (FPRECISION *)malloc(2 * config->size * sizeof(FPRECISION));
...
void fft(FPRECISION *input, FPRECISION *output, FPRECISION *twiddle_factors, int n)
{
  /*
...
void ifft(FPRECISION *input, FPRECISION *output, FPRECISION *twiddle_factors, int n)
{
  /*
...
void rfft(FPRECISION *x, FPRECISION *y, FPRECISION *twiddle_factors, int n)
{
  ...
  for (k = 2 ; k < n / 2 ; k += 2)
  {
    FPRECISION xer, xei, x0r, xoi, c, s, tr, ti;
...
void irfft(FPRECISION *x, FPRECISION *y, FPRECISION *twiddle_factors, int n)
{
  /*
...
void fft_primitive(FPRECISION *x, FPRECISION *y, int n, int stride, FPRECISION *twiddle_factors, int tw_stride)
{
  /*
  ...
  for (k = 1 ; k < n / 2 ; k++)
  {
    FPRECISION x1r, x1i, x2r, x2i, c, s;
...
void split_radix_fft(FPRECISION *x, FPRECISION *y, int n, int stride, FPRECISION *twiddle_factors, int tw_stride)
{
  /*
   * This code will compute the FFT of the input vector x
   *
  ...
  FPRECISION u1r, u1i, u2r, u2i, x1r, x1i, x2r, x2i;
  FPRECISION t;
  ...
  for (k = 1 ; k < n / 4 ; k++)
  {
    FPRECISION u1r, u1i, u2r, u2i, x1r, x1i, x2r, x2i, c1, s1, c2, s2;
...
void ifft_primitive(FPRECISION *input, FPRECISION *output, int n, int stride, FPRECISION *twiddle_factors, int tw_stride)
{
...
inline void fft8(FPRECISION *input, int stride_in, FPRECISION *output, int stride_out)
{
  /*
   * Unrolled implementation of FFT8 for a little more performance
   */
  FPRECISION a0r, a1r, a2r, a3r, a4r, a5r, a6r, a7r;
  FPRECISION a0i, a1i, a2i, a3i, a4i, a5i, a6i, a7i;
  FPRECISION b0r, b1r, b2r, b3r, b4r, b5r, b6r, b7r;
  FPRECISION b0i, b1i, b2i, b3i, b4i, b5i, b6i, b7i;
  FPRECISION t;
  //FPRECISION sin_pi_4 = 0.7071067812;
  FPRECISION sin_pi_4 = SIN_PI_BY_4;
...
inline void fft4(FPRECISION *input, int stride_in, FPRECISION *output, int stride_out)
{
  /*
